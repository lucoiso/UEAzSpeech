...
bool UWavFileToTextAsync::StartAzureTaskWork()
{
	...
	{		
		UE_LOG(LogAzSpeech_Internal, Error, TEXT("Task: %s (%d); Function: %s; Message: File not found"), *TaskName.ToString(), GetUniqueID(), *FString(__func__));
		return false;
	}
	...
	{
		UE_LOG(LogAzSpeech_Internal, Error, TEXT("Task: %s (%d); Function: %s; Message: Failed to load file"), *TaskName.ToString(), GetUniqueID(), *FString(__func__));
		return false;
	}
	else
	{
