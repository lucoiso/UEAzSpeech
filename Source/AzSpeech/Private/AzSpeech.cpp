...
FString GetRuntimeLibsDirectory()
{
    ...
    {
        UE_LOG(LogAzSpeech_Internal, Error, TEXT( "%s: Failed to get the location of the runtime libraries. Please check and validate your installation." ), *FString(__FUNCTION__));
        return FString();
    }
...
void LogLastError(const FString& FailLib)
{
    ...
    UE_LOG(LogAzSpeech_Internal, Warning, TEXT("%s: Failed to load runtime library \"%s\": %u (%s)."), *FString(__FUNCTION__), *FailLib, ErrorID, ErrorBuffer);
}
...
void FAzSpeechModule::LoadRuntimeLibraries()
{
    ...
    for (const FString& RuntimeLib : WhitelistedLibs)
    {
        ...
        for (unsigned int Attempt = 1u; !Handle && Attempt <= MaxAttempt; ++Attempt)
        {
            ...
            UE_LOG(LogAzSpeech_Internal, Display, TEXT("%s: Attempting to load runtime library \"%s\" (%u/%u)."), *FString(__FUNCTION__), *RuntimeLib, Attempt, MaxAttempt);
